...
void YGNodeStyleSetGridTemplateRows(
  ...
  if (node && trackList) {
    auto* n = node;
    n->getStyle().setGridTemplateRows(trackList->toGridTrackList());
...
void YGNodeStyleSetGridTemplateColumns(
  ...
  if (node && trackList) {
    auto* n = node;
    n->getStyle().setGridTemplateColumns(trackList->toGridTrackList());
    n->markDirtyAndPropagate();
  }
}
...
void YGNodeStyleSetGridAutoRows(YGNodeRef node, YGGridTrackListRef trackList) {
  if (node && trackList) {
    auto* n = node;
    n->getStyle().setGridAutoRows(trackList->toGridTrackList());
    n->markDirtyAndPropagate();
  }
}
...
void YGNodeStyleSetGridAutoColumns(
  ...
  if (node && trackList) {
    auto* n = node;
    n->getStyle().setGridAutoColumns(trackList->toGridTrackList());
    n->markDirtyAndPropagate();
  }
}
