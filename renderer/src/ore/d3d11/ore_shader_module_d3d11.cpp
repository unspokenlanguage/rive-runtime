...
// airz: compiled-shader cache — shared with the 2D renderer's pipeline manager,
// see rive/renderer/d3d/airz_bytecode_cache.hpp.
namespace
{
constexpr UINT kCompileFlags =
    D3DCOMPILE_ENABLE_STRICTNESS | D3DCOMPILE_OPTIMIZATION_LEVEL3;
using rive::airz::Bytecode;
} // namespace
                                             ...
                                             std::string* outError)
{
    ...
    const auto key = rive::airz::makeShaderKey(
        m_hlslSource.data(), m_hlslSource.size(), entry, target, kCompileFlags);
    Bytecode cached = cache.find(key);
    ...
    if (!cached)
    {
        ...
        HRESULT hr;
        {
            ...
            hr = D3DCompile(m_hlslSource.c_str(),
                            m_hlslSource.size(),
                            nullptr,
                            nullptr,
                            nullptr,
