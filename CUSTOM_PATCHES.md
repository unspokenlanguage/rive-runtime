...
> [!NOTE]
> **2026-09 sync (`runtime-v0.1.359`): DROPPED for a test-first build.** Upstream restructured `propagateSizeToChildren` (14 commits: `stopsContentSizing()`, `LayoutNodeProvider` participants, `shouldPropagateSizeToChildren`) and still does **not** contain this fix. It is deliberately left out so upstream's reworked layout can be tested unmodified; re-apply narrowly only if the dynamic-image-inside-a-Group test fails. The original patch is preserved in workbench branch `airz/patches-on-4a10679b`.
...
> [!NOTE]
> **2026-09 sync (`runtime-v0.1.359`): DROPPED for a test-first build.** Upstream restructured `propagateSizeToChildren` (14 commits: `stopsContentSizing()`, `LayoutNodeProvider` participants, `shouldPropagateSizeToChildren`) and still does **not** contain this fix. It is deliberately left out so upstream's reworked layout can be tested unmodified; re-apply narrowly only if the dynamic-image-inside-a-Group test fails. The original patch is preserved in workbench branch `airz/patches-on-4a10679b`.
> Note: upstream now *deliberately* blocks content-sizing at groups (`stopsContentSizing()` — "a group's contents are free content"). This is a design choice, not a missing fix; if re-applied, keep it narrow (recurse into a plain `NodeBase` group only to reach `IntrinsicallySizeable` children like `Image`; keep the `Solo` and participant rules).
...
> [!NOTE]
> **2026-09 sync (`runtime-v0.1.359`): DROPPED for a test-first build.** Upstream restructured `propagateSizeToChildren` (14 commits: `stopsContentSizing()`, `LayoutNodeProvider` participants, `shouldPropagateSizeToChildren`) and still does **not** contain this fix. It is deliberately left out so upstream's reworked layout can be tested unmodified; re-apply narrowly only if the dynamic-image-inside-a-Group test fails. The original patch is preserved in workbench branch `airz/patches-on-4a10679b`.
> Upstream still has the skip-guard (`if (m_layoutWidth != size.x || m_layoutHeight != size.y)`, ~line 233).
...
Patches upstream now covers, or that turned out to be no-ops. Retired after the 2026-09 sync to `runtime-v0.1.359`; kept for history. **Verify against the fresh upstream before trusting any entry** — this file has been wrong about this before (see Patch 5).
...
| **P1 / P2 / P3** dynamic-image-in-layout sizing | **Pending the Phase 4 test** | Dropped for a test-first build. They move here only if the "dynamic image inside a Group" test passes on unmodified upstream; otherwise they are re-applied narrowly and stay in the patch list above. |

## Sync log

| Date | From → To | `.rive_head` | Notes |
|---|---|---|---|
| 2026-09-05 | `runtime-v0.1.230` (`4a10679b`) → `runtime-v0.1.359` (`621f2a2e`), 129 upstream commits | `9e498b2635…` → `a87af1c977…` | Git 3-way rebase in the nested workbench (`UPDATING_RIVE.md` §1). Kept P5, P6. Dropped P1/P2/P3 (test-first). Yoga → `v2_0_1_3_grid` (10 grid symbols required), `rive_yoga` → C++20. Luau → `rive_0_734`. HarfBuzz stays `10.1.0` (only Apple-only `hb_ct_*` missing). Added `WITH_RIVE_SCRIPTING_LUAU`; excluded `src/wasm`. Full record: `docs/RIVE_SYNC_PLAN_2026-09.md`. |
