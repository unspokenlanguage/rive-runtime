...
- Runtime ≥ 0.1.359: the `FocusManager` lives on the artboard instance. `File::instanceArtboard` (and therefore `File::artboardDefault()`, which playout uses at load and on `__reload`) creates it and builds the focus tree; `Artboard::instance()` / `File::artboardAt()->instance()` (the editor host's path) does **not** — that side calls `ensureFocusManager()` explicitly. Focus actions, focus transition conditions, text-input and keyboard listeners silently no-op without one; nested artboards (including the state machines P5/P6 inject) inherit the parent's manager in `NestedStateMachine::initializeAnimation`. Playout guards the invariant with a warning at both creation sites (`warn_if_no_focus_manager` in `rive_producer.cpp`) so a change of load path cannot lose it unnoticed. Reported events carry `CustomProperty*` children; the editor's `bindings.json` lists them per controller.
